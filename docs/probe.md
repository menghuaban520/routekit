...
`public/routekit_probe.py` 用你电脑上已有的 mihomo 核心逐个连接节点，再从该节点发出 HTTPS 检测请求。网页不接收节点密码，也不能通过普通浏览器请求切换任意代理协议；本工具把真实节点检测留在本机。
...
本工具不测 DNS 泄漏、不覆盖所有应用、不测上传速度或丢包，也不把测速快慢转换成“纯净度”评分。节点位置之间的地理距离只能作为位置估计，不能替代实测延迟。
...
25 项离线回归使用虚构节点、mock 核心与 mock HTTPS 响应，覆盖协议转换、拒绝不支持参数、输出去凭证、实际字节计速公式、TLS / CONNECT 路径、串行选择、限流停止、部分结果与清理，以及小响应读完关闭 socket 的边界。它不证明任何私人节点可用。
