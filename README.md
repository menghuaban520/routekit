...
面向 Shadowrocket 用户的开源网页工具：导入订阅，把应用交给合适的节点，选择客户端并下载配置，再检查连接与分流结果。蓝白界面按网络检查、订阅与节点、分流配置分组，每个小工具独立显示；配置在本地生成。
...
2. **检查节点。** 在“批量实测”按教程连接本地助手；可选“导入后自动检测”，用隔离 Mihomo 逐个检测出口 IP、HTTPS 延迟与连通性，结果直接回到网页。默认不测下载，开启下载测速后每节点最多 5 MB；超过单批上限会串行分批。也可跳过检测直接配置。
3. **给应用选去向。** 点节点的“用于分流”选择默认代理，再到“分流配置”为应用选择直连、代理或拦截。代理应用可以单独指定节点，也可跟随默认代理。
4. **补充自己的应用。** 点“自定义应用”，填写名称与域名，例如 `kugou.com`；不填 `https://`、路径或端口。规则按访问域名匹配，不会自动识别设备上的应用进程。
5. **选择客户端，下载并导入。** 小火箭使用 `.conf`；出现“下载配套节点”时先导入 `.nodes.txt` 并保留 `RK_` 名称。Clash / Mihomo 使用 `.yaml`；v2rayN 使用 `.json`，通过“添加自定义配置”导入并选择 Xray 内核。各格式的操作步骤在下载按钮旁；新格式须为代理流量选定节点，不能借用小火箭当前节点。
6. **连接并验证。** 小火箭选择“配置”路由，Mihomo 客户端选择“规则”模式，v2rayN 启用自定义配置后按页面步骤手动设置应用代理。返回“网络检查”确认出口与连通性，再按泄漏检查教程核对 DNS、WebRTC 和 IPv6；到“批量检查”核对域名的规则与目标节点。
...
| 工作区 | 可以做什么 |
| --- | --- |
| 网络概览 / 速度测试 | 概览一次检查当前浏览器出口 IP、粗略位置、ASN 和三次 HTTPS 延迟；速度测试是独立页面，显示 5 MB 实际下载曲线、Mbps 与 MB/s。 |
...
| 订阅与节点 | URI / Base64 / Clash YAML 节点导入、手动添加 SS、去重、搜索、筛选、节点导出，以及将节点用于应用分流。YAML 的原规则、策略组、DNS 与远程 providers 不会导入。 |
| 套餐用量 | 读取 `Subscription-Userinfo` 中的上传、下载、额度、剩余与到期快照；可刷新最近成功读取的订阅与节点，或手动导入服务商的头信息。 |
| 批量实测 | 连接本地助手后，直接开始、查看逐节点进度、取消或恢复当前任务；使用自己的 Mihomo 核心检测出口、IP 属性、延迟、下载与估算距离，完成后排序并用于分流。也保留手动文件流程。[助手说明](docs/monitor.md) |
| 实时流量 | 本地助手只读访问已经运行的 Mihomo，展示总速度、累计计数与活跃代理链路。[助手说明](docs/monitor.md) |
...
| Clash / Mihomo YAML | 导入节点部分；导出 Mihomo 的节点、应用/自定义分流、DNS 与 Hosts。无需配套节点文件。 |
| v2rayN / Xray | 导入已支持的节点 URI；导出供 v2rayN 使用的 Xray 自定义 JSON，包含节点、路由、DNS 与本地入站。不是 v2rayN 的应用设置备份。 |

Shadowsocks 是协议，Shadowrocket、Clash 系客户端与 v2rayN 是不同客户端。每种格式使用独立导出器，不能准确映射的节点参数会明确报错并阻止导出；切换客户端不会删除原节点或小火箭专属设置。[格式与导入说明](docs/clients.md)。“代理链路”页提供客户端设置步骤；网页不会建立代理链，需在小火箭中为出口节点设置“代理通过 / Proxy Pass”并实际验证。
...
两个 Python 文件共同组成可选[本地助手](docs/monitor.md)：`routekit_monitor.py` 提供固定回环地址、会话认证、任务进度与实时监测；`routekit_probe.py` 在自己的隔离 Mihomo 内核里逐节点发出真实检测请求。网页开始检测时，所选节点凭证只发往自己的本地助手，不经 RouteKit 服务器。助手不会切换现有客户端节点或系统代理，也不提供公网代理服务。原[手动检测器流程](docs/probe.md)仍可使用。
